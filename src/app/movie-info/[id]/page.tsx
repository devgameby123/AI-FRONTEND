...
async function getData(id: string) {
  const res = await fetch(`http://54.169.232.52:8000/movies/${id}`, { cache: "no-cache" });
  return res.json();
}
async function getComment(id: string) {
  const res = await fetch(`http://54.169.232.52:8000/comment/${id}`, { cache: "no-cache" });
  return res.json();
}
async function getSentiment(id: string) {
  const res = await fetch(`http://54.169.232.52:8000/sentiment/${id}`, { cache: "no-cache" });
  return res.json();
}
...
const User = () => {
  ...
  const handleSubmit = async (commentText: string) => {
    ...
    try {
      const response = await fetch('http://54.169.232.52:8000/comment', {
        method: 'POST',
        body: formData,
      });
      if (response.ok) {
      ...
      const response2 = await fetch('http://54.169.232.52:8000/predict', {
        method: 'POST',
        body: formData2,
      });
      if (response2.ok) {
